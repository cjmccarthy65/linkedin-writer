...
async function handleGenerate({ postOnly = false } = {}) {
    ...
    try {
        ...
        const systemPrompt = `You are an expert LinkedIn ghostwriter. Write posts that stop the scroll, drive real engagement, and feel 100% human—never AI-generated.

Rules:
- Open with a bold, curiosity-driving hook (1–2 lines max, no "I'm excited to share" clichés)
...
- ${emojiNote}
- ${hashtagNote}
- NEVER start with "In today's", "I'm thrilled", or "Excited to announce"
...
"""`;
        ...
        const userPrompt = `
${role ? `AUTHOR CONTEXT:\n${role}\n` : ''}
${postTitle ? `POST TITLE: Open the post with exactly this as the bold headline: "${postTitle}"\n` : ''}
${topic ? `TOPIC: ${topic}\n` : ''}
